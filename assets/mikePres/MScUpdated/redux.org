...
** Theoretical Framework and Methodology Overview: Spectral Snapshot POD
...
<li><b>Challenge:</b> These structures are long (several pipe diameters) and wrap around the pipe circumference. </li>
...
<li><b>Why FFT-POD?</b> By exploiting the azimuthal homogeneity, the problem is decoupled, making the calculation feasible and structures clearer.</li>
</ul>
</div>
...
Key Insight (Hellström & Smits)
</div>
<p style="font-size: 1.05em; color: #34495e; margin-top: 10px;">
The largest structures in pipe flow are strongly associated with low azimuthal wavenumbers ($m$), such as $m=1$ to $m=5$.
</p>
...
<b>Snapshot Matrix:</b> $X = [\hat{u}_1,\,\hat{u}_2,\dots,\hat{u}_{N_s}] \in \mathbb{C}^{N_r \times N_s}$. The radial coordinate $r$ acts as the weight $R$ in the inner product.
</div>
</div>
...
<div style="flex: 1;">
<div style="font-size: 1.4em; font-weight: bold; color: #2c3e50; margin-bottom: 0.5em;">
Recovering Temporal Dynamics
</div>
<p style="font-size: 1.1em; color: #34495e;">
The time coefficients $b_k(t)$ (also called proper orthogonal values) represent the instantaneous amplitude of the corresponding spatial mode $\phi_k$. They are used for the final flow reconstruction.
</p>
</div>
...
<div style="font-weight: bold; color: #c0392b;">Using Eigenvectors (Your Method)</div>
#+END_EXPORT
...
<b>Implementation Note (from MATLAB code):</b> Due to the coordinate mapping or IFFT implementation, the final physical plot requires a $180^\circ$ rotation (or reflection in $y$) to align correctly with the original data, as seen in <code>plotReconstruction.m</code>.
</div>
</div>
