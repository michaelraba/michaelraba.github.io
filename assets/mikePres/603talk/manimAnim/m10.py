...
class AnandConstitutiveEvolution(MovingCameraScene):
    def construct(self):
        self.camera.frame.scale(0.8)  # Shrink frame at the start
        ...
        # Highlight part to replace in red
        self.play(
            eq1[0][13:22].animate.set_color(RED)
        )  # highlight \mathbf{T} : \mathbf{L}
        ...
        self.play(
            eq3[0][18:36].animate.set_color(YELLOW)
        )  # highlight [\tilde{\mathbf{T}}] + \mathbf{A} \dot{\theta}
        ...
        self.play(eq4[0][13:35].animate.set_color(BLUE))  # highlight first term
        self.play(eq4[0][38:].animate.set_color(GREEN))  # highlight bracketed part
        ...
        self.play(eq5[0][22:37].animate.set_color(PURPLE))  # highlight D - D^p
